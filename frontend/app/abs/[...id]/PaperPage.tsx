...
/**
 * The paper reader. Client component: it owns the fetch/poll/notify state.
 * Rendered by the server `page.tsx` next to it, which exists only to attach
 * per-paper <title>/OpenGraph metadata — `params` is the same promise the
 * server page receives, so `use()` resolves it without an extra round trip.
 */
